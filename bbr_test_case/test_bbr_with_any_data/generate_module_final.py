...
int main(int argc, char *argv[]) {{
    ...
    while (fgets(line, sizeof(line), file)) {{
        ...
        */
        ...
        // fprintf(stdout, "tp_delivered:%u, bbr->next_rtt_delivered: %u, tp_delivered_rate: %u\\n", tp_delivered, bbr->next_rtt_delivered, tp_delivered_rate);
        ...
        if (tp_rate_interval_us > 0) {{
            ...
            // u64 delivered_packets = bytes_acked / mss;

            delivered_bytes = tp_delivered_rate * (u64)BW_UNIT;

            new_bw = delivered_bytes / tp_rate_interval_us;

            // Force update if tp_rate_interval_us is lower (meaning a new valid sample)
            if (new_bw > bbr_max_bw(sk) || tp_rate_interval_us < bbr->bw.s[0].t) {{
                // printf("Updating bbr_max_bw: previous=%u, new=%llu\\n", bbr_max_bw(sk), new_bw);
